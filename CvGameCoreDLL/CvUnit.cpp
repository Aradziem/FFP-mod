...
//note TODO remove added past this if there were any
...
void CvUnit::convert(CvUnit* pUnit)
{
	...
	if ((pUnit->getExperience() * iOurModifier / iOldModifier)>=0)
	{
		setExperience((pUnit->getExperience() * iOurModifier) / iOldModifier);
	}
...
/********************************************************************************/
	...
	if (isDead())
	{
		...
		{
			float iExperience = attackXPValue();
			iExperience = (iExperience * iOurStrength) / std::max(1, iTheirStrength);
			iExperience = range(iExperience,(float) GC.getDefineINT("MIN_EXPERIENCE_PER_COMBAT"),(float) GC.getDefineINT("MAX_EXPERIENCE_PER_COMBAT"));
	...
	{
		float iExperience = pInterceptor->defenseXPValue();
	...
	{
		//Aradziem 1/5 experience from withdrawl of attacker- I think avoiding combat should also be partly rewarded.
		changeExperience((float)GC.getDefineINT("EXPERIENCE_FROM_WITHDRAWL")/5, pInterceptor->maxXPValue(), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pInterceptor->isBarbarian());
	}
...
void CvUnit::updatePlunder(int iChange, bool bUpdatePlotGroups) const
{
...
//Modified by Aradziem 8.12.24,23.12.2024
//now more elegant way
int CvUnit::cargoSpaceAvailable(SpecialUnitTypes eSpecialCargo, DomainTypes eDomainCargo) const
{
	if (specialCargo() != NO_SPECIALUNIT)
	{
		...
		{
			//Aradziem
			if (specialCargo()!= GC.getInfoTypeForString("SPECIALUNIT_ALL") && (eSpecialCargo == GC.getInfoTypeForString("SPECIALUNIT_FIGHTER") || eSpecialCargo == GC.getInfoTypeForString("SPECIALUNIT_MISSILE") || eSpecialCargo == GC.getInfoTypeForString("SPECIALUNIT_ALL")))
			{
				return 0;
			}
			//Aradziem end
...
float CvUnit::getExperience() const
{
	return m_iExperience;
}

void CvUnit::setExperience(float iNewValue, int iMax)
{
...
void CvUnit::changeExperience(float iChange, int iMax, bool bFromCombat, bool bInBorders, bool bUpdateGlobal)
{
	//Aradziem now floating point
	float iUnitExperience = iChange;
	...
	//small rounding errors reduction Aradziem
	iUnitExperience+=0.0001;
...
bool CvUnit::airStrike(CvPlot* pPlot)
{
	...
	int iTheirStrength = pDefender->currCombatStr(NULL, NULL);
	int iOurStrength = currCombatStr(NULL, NULL);
	
	if (pDefender->isDead())
	{
		//Aradziem when destroying something full experience (to make bombers experienced)
		float iExperience = pDefender->defenseXPValue();
		iExperience = (iExperience * iTheirStrength) / std::max(1, iOurStrength);
		iExperience = range(iExperience, (float)GC.getDefineINT("MIN_EXPERIENCE_PER_COMBAT"), (float)GC.getDefineINT("MAX_EXPERIENCE_PER_COMBAT"));
		changeExperience(iExperience, pDefender->maxXPValue(), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pDefender->isBarbarian());
	...
	{
		//Aradziem - 1/4 experience from regular airstrike not destroying anything.
		//Bomber squadrons tend not to get any experience from any source when there are no fighters and if there are any, bombers tend to be decimated by them.
		float iExperience = pDefender->defenseXPValue();
		//I undervalue defender strength since It's going to be much higher than ours
		iExperience = (iExperience * iTheirStrength/4) / std::max(1, iOurStrength);
		iExperience = range(iExperience, (float)GC.getDefineINT("MIN_EXPERIENCE_PER_COMBAT"), (float)GC.getDefineINT("MAX_EXPERIENCE_PER_COMBAT"));
		if(iExperience>1)
		{
			iExperience=1;
		}
		changeExperience(iExperience/4, pDefender->maxXPValue(), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pDefender->isBarbarian());
	}
	return true;
}
