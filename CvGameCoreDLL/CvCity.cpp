...
void CvCity::init(int iID, PlayerTypes eOwner, int iX, int iY, bool bBumpUnits, bool bUpdatePlotGroups)
{
	...
	if (pPlot->getPlotSolarSystem() != NULL)
	{
		m_iSolarSystemID = pPlot->getPlotSolarSystem()->getID();
		pPlot->getPlotSolarSystem()->setCity(getIDInfo());
	}
...
void CvCity::kill(bool bUpdatePlotGroups, bool bRemoveCulture) // DarkLunaPhantom - Added bRemoveCulture to erase (most) plot culture of destroyed cities.
{
    ...
    if (bRemoveCulture)
    {
        for (int iI=0; iI < MAX_PLAYERS; ++iI)//Aradziem Someone forgot =0
        {
...
void CvCity::updateYield()
{
	...
	GC.getMap().getSolarSystem(m_iSolarSystemID)->updateYield();
}
...
int CvCity::getCitySolarSystemID()
{
	return m_iSolarSystemID;
}
void CvCity::setCitySolarSystem(int id)
{
	m_iSolarSystemID = id;
}
...
void CvCity::addProductionExperience(CvUnit* pUnit, bool bConscript)
{
	...
	{
		pUnit->changeExperience(getProductionExperience(pUnit->getUnitType()) / ((bConscript) ? 2.0f : 1.0f));
	}
...
void CvCity::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(m_iX);
	pStream->Write(m_iY);
	pStream->Write(m_iSolarSystemID);
