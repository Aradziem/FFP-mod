...
void CyMap::generateSolarSystems()
{
	if (m_pMap)
	{
		m_pMap->generateSolarSystems();
	}
}

int CyMap::getNumSolarSystems()
{
	if (m_pMap)
	{
		return m_pMap->getNumSystems();
	}
	return -1;
}

void CyMap::resetSolarSystems()
{
	if (m_pMap)
	{
		m_pMap->resetSystems();
...
int CyMap::getSolarSystemID(int x, int y)
{
	if (m_pMap)
	{
		return m_pMap->getSolarSystemAt(x, y)->getID();
	}
	return -1;
}

CySolarSystem* CyMap::getSystem(int ID)
{
	return m_pMap ? new CySolarSystem(m_pMap->getSolarSystem(ID)) : NULL;
}
