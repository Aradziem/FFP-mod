...
bool CvBuildingInfo::isArtTagSizeDependent()
{
	return m_bSizeDependentArtTag;
}

CvString CvBuildingInfo::getPlanetSizeArtTag(PlanetSizes i)
{
	return m_pszPlaneSizeArtTags[i];
}
//End of Final Frontier
...
//Added in Final Frontier SDK: TC01
	...
	if (!pXML->GetChildXmlValByName(&m_bSizeDependentArtTag, "bPlanetSizeDependentArtTag"))
	{
		m_bSizeDependentArtTag = false;
	}
	pXML->SetVariableListTagPair(&m_pszPlaneSizeArtTags, "PlanetSizeArtTags",GC.getNumPlanetSizeInfos());
//End of Final Frontier SDK
...
int CvProjectInfo::getNumStageArtDefineTags() 
{
	return m_iNumStageArtDefineTags;
}

int* CvProjectInfo::getStageForArtDefineTag()
{
	return m_piStageForArtDefineTag;
}

CvString* CvProjectInfo::getStageArtDefineTags()
{
	return m_pszStageArtDefineTags;
}
...
bool CvProjectInfo::read(CvXMLLoadUtility* pXML)
{
	...
	//Aradziem
	// get the total number of children the current xml node has
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "PlanetSizePopulationArtTags"))
	{
		//gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
		m_iNumStageArtDefineTags = gDLL->getXMLIFace()->GetNumChildren(pXML->GetXML());
		m_piStageForArtDefineTag = new int[m_iNumStageArtDefineTags];
		m_pszStageArtDefineTags = new CvString[m_iNumStageArtDefineTags];
		if ((0 < m_iNumStageArtDefineTags) && (gDLL->getXMLIFace()->SetToChild(pXML->GetXML())))
		{
			// loop through all the siblings
			for (int i = 0;i < m_iNumStageArtDefineTags;i++)
			{
				if (pXML->GetChildXmlVal(&m_piStageForArtDefineTag[i]))
				{
					pXML->GetNextXmlVal(m_pszStageArtDefineTags[i]);
					// set the current xml node to it's parent node
					gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
				}
				// if the call to the function that sets the current xml node to it's first non-comment
...
CvPlanetInfo::~CvPlanetInfo()
{
	SAFE_DELETE_ARRAY(m_piYieldInfo);
}
...
bool CvPlanetInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(m_szDescription, "Description");
	pXML->GetChildXmlValByName(m_szCivilopedia, "Civilopedia");
	pXML->GetChildXmlValByName(m_iArtDefineTag, "ArtDefineTag");
	pXML->GetChildXmlValByName(szTextVal, "Class");
	m_iClass = pXML->FindInInfoClass(szTextVal);
	//m_iClass = pXML->FindInInfoClass(szTextVal);
	pXML->GetChildXmlValByName(&b_gasGiant, "bGasGiant");
	pXML->GetChildXmlValByName(szTextVal, "MinSize");
	m_iMinSize = pXML->FindInInfoClass(szTextVal);
	pXML->GetChildXmlValByName(szTextVal, "MaxSize");
	m_iMaxSize = pXML->FindInInfoClass(szTextVal);
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "Yields"))
	{
		// call the function that sets the yield change variable
		pXML->SetYields(&m_piYieldInfo);
		gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
	}
	else
	{
		pXML->InitList(&m_piYieldInfo, NUM_YIELD_TYPES);//Not sure why would someone add a planet without yields
		//but I allow this in case this is intentional(e.g. base yields depend on something).
	}
	pXML->GetChildXmlValByName(&m_iHealthPerPop, "iHealthPerPop");
	pXML->GetChildXmlValByName(&m_iHappinessPerPop, "iHappinessPerPop");

	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "PlanetFeatures"))
	{
		
		m_iNumBaseFeatures = gDLL->getXMLIFace()->GetNumChildren(pXML->GetXML());
		m_piBaseFeatures = new int[m_iNumBaseFeatures];
		if ((0 < m_iNumBaseFeatures) && (gDLL->getXMLIFace()->SetToChild(pXML->GetXML())))
		{
			// loop through all the siblings
			for (int i = 0;i < m_iNumBaseFeatures;i++)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					m_piBaseFeatures[i] = pXML->FindInInfoClass(szTextVal);
					gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
				}
				// if the call to the function that sets the current xml node to it's first non-comment
				// sibling and sets the parameter with the new node's value does not succeed
	...
	{
		m_iNumBaseFeatures = 0;
	}

	return true;
}

/*CvString CvPlanetInfo::getType()//probably should be int or special type
{
	return m_szType;
}*/

CvString CvPlanetInfo::getName()
{
	return m_szName;
}

CvString CvPlanetInfo::getDescription()
{
	return m_szDescription;
}

CvString CvPlanetInfo::getCivilopedia()
{
	return m_szCivilopedia;
}

CvString CvPlanetInfo::getArtDefineTag() const
{
	return m_iArtDefineTag;
}

int CvPlanetInfo::getClass() const
{
	return m_iClass;
}

bool CvPlanetInfo::isGasGiant() const
{
	return b_gasGiant;
}

int CvPlanetInfo::getMinSize() const
{
	return m_iMinSize;
}
int CvPlanetInfo::getMaxSize() const
{
	return m_iMaxSize;
}

int CvPlanetInfo::getYield(int i)
{
	return m_piYieldInfo[i];
}

int CvPlanetInfo::getHealthPerPop() const
{
	return m_iHealthPerPop;
}

int CvPlanetInfo::getHappinessPerPop() const
{
	return m_iHappinessPerPop;
}

int CvPlanetInfo::getNumBaseFeatures() const
{
	return m_iNumBaseFeatures;
}

int CvPlanetInfo::getBaseFeature(int i)
{
	return m_piBaseFeatures[i];
}

CvOrbitInfo::CvOrbitInfo() :
	m_iOrbitArtDefineTag(0),
	m_iPlanettArtDefineTag(0),
	m_iBuildingArtDefineTag(0)
{
}

//------------------------------------------------------------------------------------------------------
//
//  FUNCTION:   ~CvOrbitInfo()
//
//  PURPOSE :   Default destructor
//
//------------------------------------------------------------------------------------------------------
...
bool CvOrbitInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
	...
	//pXML->GetChildXmlValByName(m_szType, "Type");
	pXML->GetChildXmlValByName(m_iOrbitArtDefineTag, "OrbitArtTag");
	pXML->GetChildXmlValByName(m_iPlanettArtDefineTag, "PlanetArtTag");
	pXML->GetChildXmlValByName(m_iBuildingArtDefineTag, "BuildingArtTag");

	return true;
}

CvString CvOrbitInfo::getOrbitArtDefineTag() const
{
	return m_iOrbitArtDefineTag;
}
CvString CvOrbitInfo::getPlanetArtDefineTag() const
{
	return m_iPlanettArtDefineTag;
}
CvString CvOrbitInfo::getBuildingArtDefineTag() const
{
	return m_iBuildingArtDefineTag;
}

CvPlanetSizeInfo::CvPlanetSizeInfo() :
	m_iPopulation(0),
	m_iPlanetSizeArtTag(0),
	//m_piPlanetSizeArtTagTypes(NULL),
	m_piPlanetSizeArtTags(NULL),
	m_piPlanetSizePopulationForArtTag(NULL),
	m_piPlanetSizePopulationArtTags(NULL)
{
}
...
CvPlanetSizeInfo::~CvPlanetSizeInfo()
{
	//SAFE_DELETE_ARRAY(m_piPlanetSizeArtTagTypes);
	SAFE_DELETE_ARRAY(m_piPlanetSizeArtTags);
	SAFE_DELETE_ARRAY(m_piPlanetSizePopulationForArtTag);
	SAFE_DELETE_ARRAY(m_piPlanetSizePopulationArtTags);
}
...
bool CvPlanetSizeInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(m_iPlanetSizeArtTag, "PlanetSizeModel");

	

	pXML->SetVariableListTagPair(&m_piPlanetSizeArtTags, "PlanetSizeArtTags", /*sizeof(GC.getCivilizationInfo((CivilizationTypes)0)),*/ GC.getNumPlanetSizeArtTagTypeInfos());

	/*
	// get the total number of children the current xml node has
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "PlanetSizeArtTags"))
	{
		gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
		m_iNumPlanetSizeArtTagTypes = gDLL->getXMLIFace()->GetNumChildren(pXML->GetXML());
		m_piPlanetSizeArtTagTypes = new int[m_iNumPlanetSizeArtTagTypes];
		m_piPlanetSizeArtTags = new CvString[m_iNumPlanetSizeArtTagTypes];
		int* iTemp = new int[m_iNumPlanetSizeArtTagTypes];
		CvString* sTemp = new CvString[m_iNumPlanetSizeArtTagTypes];
		if ((0 < m_iNumPlanetSizeArtTagTypes) && (gDLL->getXMLIFace()->SetToChild(pXML->GetXML())))
		{
			// loop through all the siblings
			for (int i = 0;i < m_iNumPlanetSizeArtTagTypes;i++)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					iTemp[i] = pXML->FindInInfoClass(szTextVal);
					pXML->GetNextXmlVal(sTemp[i]);
					// set the current xml node to it's parent node
					gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
				}
				// if the call to the function that sets the current xml node to it's first non-comment
	...
	*/
	...
	{
		//gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
		m_iNumPlanetSizePopulationArtTagTypes = gDLL->getXMLIFace()->GetNumChildren(pXML->GetXML());
		m_piPlanetSizePopulationForArtTag = new int[m_iNumPlanetSizePopulationArtTagTypes];
		m_piPlanetSizePopulationArtTags = new CvString[m_iNumPlanetSizePopulationArtTagTypes];
		if ((0 < m_iNumPlanetSizePopulationArtTagTypes) && (gDLL->getXMLIFace()->SetToChild(pXML->GetXML())))
		{
			// loop through all the siblings
			for (int i = 0;i < m_iNumPlanetSizePopulationArtTagTypes;i++)
			{
				if (pXML->GetChildXmlVal(&m_piPlanetSizePopulationForArtTag[i]))
				{
					pXML->GetNextXmlVal(m_piPlanetSizePopulationArtTags[i]);
					// set the current xml node to it's parent node
					gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
				}
				// if the call to the function that sets the current xml node to it's first non-comment
...
CvString CvPlanetSizeInfo::getName()
{
	return m_szName;
}

CvString CvPlanetSizeInfo::getDescripyion()
{
	return m_szDescription;
}

CvString CvPlanetSizeInfo::getCivilopedia()
{
	return m_szCivilopedia;
}

int CvPlanetSizeInfo::getPopulation() const
{
	return m_iPopulation;
}

CvString CvPlanetSizeInfo::getPlanetSizeArtTag() const
{
	return m_iPlanetSizeArtTag;
}

/*int CvPlanetSizeInfo::getNumPlanetSizeArtTagTypes() const
{
	return m_iNumPlanetSizeArtTagTypes;
}

int* CvPlanetSizeInfo::getPlanetSizeArtTagTypes()//i.e. category of art tag
{
	return m_piPlanetSizeArtTagTypes;
}*/

CvString CvPlanetSizeInfo::getPlanetSizeArtTags(int i)//the art tag used for that size
{
	FAssertMsg(i < GC.getNumPlanetSizeArtTagTypeInfos(), "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	return m_piPlanetSizeArtTags[(int)i];
}

int CvPlanetSizeInfo::getNumPlanetSizePopulationArtTagTypes() const
{
	return m_iNumPlanetSizePopulationArtTagTypes;
}

int* CvPlanetSizeInfo::getPlanetSizePopulationForArtTags()//minimum population for that art tag (highest value below or equal to should be used)
{
	return m_piPlanetSizePopulationForArtTag;
}

CvString* CvPlanetSizeInfo::getPlanetSizePopulationArtTags()//art tag for that population
{
	return m_piPlanetSizePopulationArtTags;
}

CvPlanetSizeArtTagTypeInfo::CvPlanetSizeArtTagTypeInfo() 
{
}

//------------------------------------------------------------------------------------------------------
//
//  FUNCTION:   ~CvPlanetSizeArtTagTypeInfo()
//
//  PURPOSE :   Default destructor
//
//------------------------------------------------------------------------------------------------------
...
bool CvPlanetSizeArtTagTypeInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
...
bool CvStarInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
	...
	pXML->GetChildXmlValByName(m_szDescription, "Description");
	pXML->GetChildXmlValByName(m_szCivilopedia, "Civilopedia");
	pXML->GetChildXmlValByName(m_iArtDefineTag, "ArtTag");
	return true;
}
...
CvString CvStarInfo::getName()
{
	return m_szName;
}

CvString CvStarInfo::getDescription()
{
	return m_szDescription;
}

CvString CvStarInfo::getCivilopedia()
{
	return m_szCivilopedia;
}

CvString CvStarInfo::getArtDefineTag() const
{
	return m_iArtDefineTag;
}
//Aradziem end
