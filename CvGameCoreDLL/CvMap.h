...
public:
	...
	//Aradziem
	CvSolarSystem* getSolarSystem(int iID);
	CvSolarSystem* getSolarSystemAt(int iX,int iY);
	void addSolarSystem(CvSolarSystem* pSystem);
	void growSolarSystemsTable();
	int getFreeSolarSystemIndex();
	void setSolarSystem(CvSolarSystem* system, int iID);
	void resetSystems();
	void deleteSolarSystem(int iID);
	//Aradziem end
...
/************************************************************************************************/
	...
	int getRandomPlanetNames0() const;
	int getRandomPlanetNames1() const;
	int getRandomPlanetNames2() const;
	int getRandomPlanetNames3() const;
	CvWString* getStringRandomPlanetNames0();
	CvWString* getStringRandomPlanetNames1();
	CvWString* getStringRandomPlanetNames2();
	CvWString* getStringRandomPlanetNames3();

	//#iMaxDefaultPopulationPerPlanet = 1


	//#aiPlanetTypes.append(iPlanetTypeGreen)
	//solar systems functions

	bool isPlotAPlayersStart(CvPlot* pPlot);

	int** getRandomizedPlanetInfo(int iNumPlanets, YieldTypes iPreferredYield);
	PlanetTypes getRandomPlanetType(YieldTypes iPreferredYield, PlanetSizes iSize);

	int* getRandomizedOrbitRingArray(int iNumPlanets);

	int getNumSystems() const;
	//CvSolarSystem* getSystem(int iSystemID);
	//CvSolarSystem* getSystemAt(int iX, int iY);
	FeatureTypes getFeatureIDSolarSystem();

	int getPlanetRange2() const;
	int getPlanetRange3() const;
	CvSolarSystem* createRandomSystem(int iX, int iY, YieldTypes iPreferredYield, int iPreferredPlanetQuantity);
	void generateSolarSystems();//exposed to python
	void AddPlanetaryResources();
...
protected:
	...
	//Aradziem
	int i_numSolarSystemsSlots;
	CvSolarSystem** m_solarSystems;
	//Aradziem end

	void calculateAreas();
	//Aradziem
	//solar system globals - declared here so as not to have them more than once
	FeatureTypes m_iFeatureIDSolarSystem;

	int m_g_iPlanetRange2;
	int m_g_iPlanetRange3;

	int m_i_RandomPlanetNames0;
	int m_i_RandomPlanetNames1;
	int m_i_RandomPlanetNames2;
	int m_i_RandomPlanetNames3;
	CvWString m_g_aszRandomPlanetNames0[46];
	CvWString m_g_aszRandomPlanetNames1[41];
	CvWString m_g_aszRandomPlanetNames2[22];
	CvWString m_g_aszRandomPlanetNames3[24];
	//Aradziem end
