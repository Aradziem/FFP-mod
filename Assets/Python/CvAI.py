...
##########################################################
	...
	def findBestChokepoint(self, iPlayer=-1, bCheckForVisibility=false):
		...
		for iPlotLoop in range(CyMap().numPlots()):
			...
			if (pPlayer.getCapitalCity().plot()!=None):#Aradziem bugfix (was always true)
				iDistanceFromCapital = CyMap().calculatePathDistance(pPlayer.getCapitalCity().plot(), pLoopPlot)#TODO fix this, breaks down when sb has no capital
