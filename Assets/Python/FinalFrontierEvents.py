...
class FinalFrontierEvents(CvEventManager.CvEventManager):
	...
	def onGameStart(self, argsList):
		...
		# Debug stuff
		#for iSystemLoop in range(CyMap().getNumSystems()): #FFPBUG
			#printd("System (%d, %d) Num Planets: %d" %(CyMap().getSystem(iSystemLoop).getX(), CyMap().getSystem(iSystemLoop).getY(), CyMap().getSystem(iSystemLoop).getNumPlanets())) #FFPBUG
...
#############################################################################################
	...
	def onModNetMessage(self, argsList):
		...
		if (iMessage == self.iNetMessage_setSelectedPlanet):
			...
			pSystem = CyMap().getSolarSystemID(iX, iY) #FFPBUG
			CySolarSystem = CyMap().getSolarSystem(pSystem)
			
			CySolarSystem.setSelectedPlanetRing(iPlanetRing)
			
			CySolarSystem.updateDisplay()
		...
		# Assign 1 population to a planet
			...
			self.doAddPopulationToPlanet(CySolarSystem, iPlanetRing)
...
#		self.iNumSystems = 0
	...
	def updateSystemsDisplay(self):
		...
		for iSystemLoop in range(CyMap().getNumSystems()): #FFPBUG
			CyMap().getSolarSystem(iSystemLoop).updateDisplay() #FFPBUG
		
	def updateNeededSystemsDisplay(self):
		#Aradziem modified this function
		for iSystemLoop in range(CyMap().getNumSystems()): #FFPBUG
			#pSystem = CyMap().getSystem(iSystemLoop) #Aradziem
			CySolarSystem = CyMap().getSolarSystem(iSystemLoop)
			if (CySolarSystem.getNeedsUpdate()):
				CySolarSystem.updateDisplay()
				CySolarSystem.setNeedsUpdate(false)
...
#############################################################################################
	...
	def initScoreStuff(self):
		...
		for iSystemLoop in range(CyMap().getNumSystems()): #FFPBUG
			CySolarSystem = CyMap().getSolarSystem(iSystemLoop) #FFPBUG
			
			for iPlanetLoop in range(CySolarSystem.getNumPlanets()):
				CyPlanet = CySolarSystem.getPlanet(iPlanetLoop)
				
				iMaxFood += CyPlanet.getYield(-1,0) # Add up the Food.. mmmm
	...
	def onMouseEvent(self, argsList):
		...
		if ( px!=-1 and py!=-1 ):
			...
			if ( eventType == self.EventLcButtonDblClick ):
				...
				for iSystemLoop in range(CyMap().getNumSystems()): #FFPBUG
					CySolarSystem = CyMap().getSolarSystem(iSystemLoop)
					#pSystem = CyMap().getSystem(iSystemLoop) #FFPBUG
					pPlot = CyMap().plot(CySolarSystem.getX(),CySolarSystem.getY())
					pCity = pPlot.getPlotCity()
					...
					if (szTagName != ""):
						...
						if (iPlanetRing != -1):
							...
							printd(iPlanetRing)
							printd("System (%d, %d) Num Planets %d" %(CySolarSystem.getX(), CySolarSystem.getY(), CySolarSystem.getNumPlanets()))
							for i in range(CySolarSystem.getNumPlanets()):
								#pPlanet = CyMap().getSolarSystem(iSystemLoop).getPlanetIDByIndex(i)
								printd("Planet ID %d's orbit ring %d" %(i,CySolarSystem.getPlanet(i).getRing()))
							...
							if (pPlanet != -1):
								...
								CyMessageControl().sendModNetMessage(self.iNetMessage_setSelectedPlanet, CySolarSystem.getX(),CySolarSystem.getY(), iPlanetRing, -1)
