...
# BUG - Build/Action Icon Size - end
	...
	def addPlanetCustomization(self, CySolarSystem, CyPlanet, szWidgetName, bShowSelection = true):
		...
		if (CyPlanet.isDisabled()):
			...
			if (bShowSelection):
				if (CyInterface().isCityScreenUp()):
					if (CySolarSystem.getSelectedPlanetRing() == CyPlanet.getRing()):
						szPlanetArt = aszPlanetSelectionSizes[CyPlanet.getSize()]
						szFilename = ArtFileMgr.getFeatureArtInfo("ART_DEF_FEATURE_SOLAR_SYSTEM").getFeatureDummyNodeName(0, szPlanetArt)
						screen.addToModelGraphicGFC(szWidgetName, szFilename)
# BUG - city specialist - start
...
#################################################################################################
		...
		if (pHeadSelectedCity):
			...
			pSystem = CyMap().getSolarSystemID(pHeadSelectedCity.getX(), pHeadSelectedCity.getY()) #FFPBUG

			if (pSystem!=-1):
				CySolarSystem = CyMap().getSolarSystem(pSystem)
				# Update planet selection graphics
				#pSystem.updateDisplay()
				CySolarSystem.setNeedsUpdate(True)
				...
				if (CyInterface().isCityScreenUp()):
					...
					iSelectedRing = CySolarSystem.getSelectedPlanetRing()
					
					printd("Default Selected Ring is %d" %(iSelectedRing))
					
					if (iSelectedRing == -1):
						iSelectedRing = 0
						CySolarSystem.setSelectedPlanetRing(iSelectedRing)


					pPlanet = CySolarSystem.getPlanetIDByRing(iSelectedRing)

					
					printd(pPlanet)
					
					if (pPlanet != -1):
						CyPlanet = CySolarSystem.getPlanet(pPlanet)
	#					szText = "%s %d: %s" %(pHeadSelectedCity.getName(), iSelectedRing, aszPlanetTypeNames[pPlanet.getPlanetType()])
						...
						if CyPlanet.isBonus(): # Planetary Resource Indicator
							szText += u" %c" % gc.getBonusInfo(CyPlanet.getBonusType()).getChar() # Planetary Resource Indicator
						...
						self.addPlanetCustomization(CySolarSystem, CyPlanet, "SelectedPlanetGraphic", false)
...
#						screen.addToModelGraphicGFC("SelectedPlanetGraphic", filename)
						...
						# Selected Planet Pop
						szPopulation = "%d/%d" %(CyPlanet.getPopulation(), CyPlanet.getPopulationLimit(pHeadSelectedCity.getOwner()))
						...
						# Planet yield
						iFood = CyPlanet.getYield(pHeadSelectedCity.getOwner(), 0)
						iProd = CyPlanet.getYield(pHeadSelectedCity.getOwner(), 1)
						iCom = CyPlanet.getYield(pHeadSelectedCity.getOwner(), 2)
						...
						iMax = CySolarSystem.getPopulationLimit(true)
						iUnassigned = iMax
						for iPlanetLoop in range(CySolarSystem.getNumPlanets()):
							iUnassigned -= CySolarSystem.getPlanet(iPlanetLoop).getPopulation()
						...
						bState = false
						if (CySolarSystem.getBuildingPlanetRing() == iSelectedRing):
							bState = true
						...
						if (CyPlanet.isWithinCulturalRange()):
							...
							for iBuildingLoop in range(gc.getNumBuildingInfos()):
								...
								if (CyPlanet.isBuilding(iBuildingLoop)):
					...
					for iPlanetLoop in range(0,8):
						...
						pLoopPlanet = CySolarSystem.getPlanetIDByRing(iPlanetLoop)
						...
						if (pLoopPlanet != -1):
							...
							szText = "%d/%d" %(CyPlanet.getPopulation(), CyPlanet.getPopulationLimit(pHeadSelectedCity.getOwner()))
							
							if (CySolarSystem.getBuildingPlanetRing() == iPlanetLoop):
								szText += " B"
...
# BUG - field of view slider - end
		...
		if (pCity):
			...
			pSystem = CyMap().getSolarSystemID(pCity.getX(), pCity.getY()) #FFPBUG
			CySolarSystem = CyMap().getSolarSystem(pSystem)
			iPlanetID = CySolarSystem.getSelectedPlanetID()
			#CyPlanet = CySolarSystem.getPlanet(iPlanetID)
			
			# Select a planet
			if (inputClass.getFunctionName() == "PlanetGraphic"):
				
				CyMessageControl().sendModNetMessage(FinalFrontier.iNetMessage_setSelectedPlanet, CySolarSystem.getX(), CySolarSystem.getY(), inputClass.getID(), -1)
				
			if (iPlanetID != -1):#Aradziem not exactly the same check
				
				# Increase Planet Population Button
				if (inputClass.getFunctionName() == "PlanetPopulationIncrease"):
					
					CyMessageControl().sendModNetMessage(FinalFrontier.iNetMessage_addPopulation, CySolarSystem.getX(), CySolarSystem.getY(), iPlanetID, -1)
				
				# Decrease Planet Population Button
				elif (inputClass.getFunctionName() == "PlanetPopulationDecrease"):
					
					if (CyPlanet.getPopulation() > 0):
						iRemove = -1
						CyMessageControl().sendModNetMessage(FinalFrontier.iNetMessage_RemovePopulation, CySolarSystem.getX(), CySolarSystem.getY(), iPlanetID, iRemove)
						
				# Assign Building to Planet Button
				elif (inputClass.getFunctionName() == "PlanetAssignConstruction"):
					
					if (CySolarSystem.getBuildingPlanetID() != iPlanetID):

						CyMessageControl().sendModNetMessage(FinalFrontier.iNetMessage_AssignBuilding, CySolarSystem.getX(), CySolarSystem.getY(), iPlanetID, -1)
