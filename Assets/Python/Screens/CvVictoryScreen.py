...
# BUG Additions Start
				if AdvisorOpt.isVictories():
					...
					for i in range(gc.getNumProjectInfos()):
						#if (gc.getProjectInfo(i).getVictoryThreshold(iLoopVC, -1) > 0):
						if (gc.getProjectInfo(i).getVictoryThreshold(iLoopVC) > 0):#Aradziem not sure why it works now, what was purpose of this -1 int argument?
							iRow = screen.appendTableRow(szTable)
							#if (gc.getProjectInfo(i).getVictoryMinThreshold(iLoopVC, iActiveTeam) == gc.getProjectInfo(i).getVictoryThreshold(iLoopVC, iActiveTeam)):#Aradziem
							if (gc.getProjectInfo(i).getVictoryMinThreshold(iLoopVC) == gc.getProjectInfo(i).getVictoryThreshold(iLoopVC)):
								#szNumber = unicode(gc.getProjectInfo(i).getVictoryThreshold(iLoopVC, iActiveTeam))#Aradziem
								szNumber = unicode(gc.getProjectInfo(i).getVictoryThreshold(iLoopVC))
							else:
								#szNumber = unicode(gc.getProjectInfo(i).getVictoryMinThreshold(iLoopVC, iActiveTeam)) + u"-" + unicode(gc.getProjectInfo(i).getVictoryThreshold(iLoopVC, iActiveTeam))#Aradziem
...
# BUG Additions End
				...
				if (victory.getCityCulture() != CultureLevelTypes.NO_CULTURELEVEL and victory.getNumCultureCities() > 0):#Aradziem again
					...
					iRow = screen.appendTableRow(szTable)
					#screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_CITY_CULTURE", (victory.getNumCultureCities(iActiveTeam), gc.getCultureLevelInfo(victory.getCityCulture()).getTextKey())), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)#This construtor takes no arguments?
					...
					#for i in range(max(victory.getNumCultureCities(iActiveTeam), victory.getNumCultureCities(iBestCultureTeam))):This constructor takes no arguments
					for i in range(max(victory.getNumCultureCities(), victory.getNumCultureCities())):	
...
# BUG Additions End
						...
						#if (i < max(victory.getNumCultureCities(iActiveTeam), victory.getNumCultureCities(iBestCultureTeam)) - 1):constructor again
						if (i < max(victory.getNumCultureCities(), victory.getNumCultureCities()) - 1):
							iRow = screen.appendTableRow(szTable)
					bEntriesFound = True
...
# rewritten for K-Mod
	def getListCultureCities(self, iTeam, victory):
		...
		return cultureCityList[0:victory.getNumCultureCities()]#Aradziem constructor takes no arguments?
