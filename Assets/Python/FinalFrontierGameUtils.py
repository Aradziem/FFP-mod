...
class FFGameUtils:
	...
	def cannotConstruct(self,argsList):
		...
		#Aradziem - changes in this function. I think it ought to be moved to c++ because it is quite frequently called
		pSolarSystem = CyMap().getSolarSystemID(pCity.getX(), pCity.getY()) #FFPBUG
		CySolarSystem = CyMap().getSolarSystem(pSolarSystem)
		pPlanet = CySolarSystem.getPlanetIDByRing(CySolarSystem.getBuildingPlanetRing())
		...
		if (pPlanet>=0):#Aradziem changed this
			if CyMap().isBuilding(pSystem,pPlanet,eBuilding):
				bNotHere = True
		else:
			return True # this should never happen
		
		#Moon and Rings restrictors moved to XML tags: TC01
		if (pBuildingInfo.isMoon()) and (not CyPlanet.isMoon()):
			bNotHere = true
			
		if (pBuildingInfo.isRings()) and (not CyPlanet.isRings()):
			bNotHere = true

		if (eBuilding in self.dBuildingBonuses):
			if not CyPlanet.isBonus():
				bNotHere = True
			elif CyPlanet.getBonus() != self.dBuildingBonuses[eBuilding]:
		...
		if (not pBuildingInfo.isNukeImmune()) and CyPlanet.isDisabled():
			bNotHere = True
		...
		for iNeededBuildingClassLoop in range(gc.getNumBuildingClassInfos()):
			if (pBuildingInfo.isBuildingClassNeededOnPlanet(iNeededBuildingClassLoop)):
				...
				if (not CyPlanet.isBuilding(iNeededBuildingLoop)):
					bNotHere = True
					break
		...
		if bNotHere and (not gc.getPlayer(pCity.getOwner()).isHuman()):
			for iPlanetLoop in range(CySolarSystem().getNumPlanets(pSystem)):
				if (iPlanetLoop != pPlanet) and (CyPlanet().isWithinCulturalRange(iPlanetLoop)) :
					# skip the planet we are already using and planets not in the cultural range
					
					if (pBuildingInfo.isMoon()) and (not CyMap().isMoon(pSystem,iPlanetLoop)):
						continue
						
					if (pBuildingInfo.isRings()) and (not CyMap().isRings(pSystem,iPlanetLoop)):
						continue
						
					if CyMap().isBuilding(pSystem,iPlanetLoop,eBuilding):
						continue # already has one here, skip rest of loop
					
					if (eBuilding in self.dBuildingBonuses):
						if not CyMap().isBonus(pSystem,iPlanetLoop):
							# planet does not have bonus but one is required by this building
							continue
						elif (CyMap().getPlanetBonus(pSystem,iPlanetLoop) != self.dBuildingBonuses[eBuilding]):
							# the planet has a bonus, but it is the wrong one - since there can be only one bonus in a system
							# and we know it has the wrong one, this building can not be built anywhere in this system
							return True
					
					if (not pBuildingInfo.isNukeImmune()) and CyMap().isDisabled(pSystem,iPlanetLoop):
						continue
					...
					for iNeededBuildingClassLoop in range(gc.getNumBuildingClassInfos()):
						if (pBuildingInfo.isBuildingClassNeededInCity(iNeededBuildingClassLoop)):
							...
							if not CyMap().isHasBuilding(pSystem,iPlanetLoop,iNeededBuildingLoop):
								hasPrereq = False
					...
					if (hasPrereq == True) :
						...
						if (not bTestVisible) : # only actually change the ring if not just testing "visibility"
							CyMap().setBuildingPlanetRing(pSystem,CyMap().getPlanetRing(pSystem,iPlanetLoop)) # switch to this new planet
							CvSolarSystem.printd("CvGameUtils.cannotConstruct, %s active planet ring set to %d (turn = %d, owner = %s, building = %s, bContinue = %d, bTestVisible = %d)" % (pCity.getName(), CyMap().getPlanetRing(pSystem,iPlanetLoop), CyGame().getElapsedGameTurns(), gc.getPlayer(pCity.getOwner()).getName(), pBuildingInfo.getType(), bContinue, bTestVisible) )
						else:
...
## Civics Screen ##
		...
		if (eWidgetType == WidgetTypes.WIDGET_GENERAL):
			...
			if (iData1 >= 671 and iData1 <= 678):
				#Aradziem modified this function
				...
				pPlanet = CySolarSystem.getPlanetIDByRing(iPlanetRing)
				CyPlanet = CySolarSystem.getPlanet(pPlanet)
				iFood = CyPlanet.getYield(pHeadSelectedCity.getOwner(), 0)
				iProduction = CyPlanet.getYield(pHeadSelectedCity.getOwner(), 1)
				iCommerce = CyPlanet.getYield(pHeadSelectedCity.getOwner(), 2)
				
				iCultureLevel = CyPlanet.getCulturalRange()

				if CyPlanet.isBonus():
					szHelpText = CyPlanet.getName() + "\n" + localText.getText("TXT_KEY_FF_INTERFACE_PLANET_SELECTION_HELP_0", (iFood, iProduction, iCommerce)) + "\n" + localText.getText("TXT_KEY_FFPLUS_PLANET_SELECTION_HELP", (iCultureLevel,)) + "\n" + u"%c " % gc.getBonusInfo(CyPlanet.getBonus()).getChar() + localText.getText("TXT_KEY_FFPLUS_PLANET_SELECTION_BONUS", (gc.getBonusInfo(CyPlanet.getBonus()).getDescription(),)) # Planetary Resource Indicator
				else:
					szHelpText = CyPlanet.getName() + "\n" + localText.getText("TXT_KEY_FF_INTERFACE_PLANET_SELECTION_HELP_0", (iFood, iProduction, iCommerce)) + "\n" + localText.getText("TXT_KEY_FFPLUS_PLANET_SELECTION_HELP", (iCultureLevel,))
				if (not CyUserProfile().getPlayerOption(PlayerOptionTypes.PLAYEROPTION_MODDER_1)):
